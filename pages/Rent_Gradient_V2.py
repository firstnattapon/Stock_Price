...
def _render_sidebar_cbd_panel(locked: bool) -> Tuple[bool, bool]:
    ...
    with st.expander("🎯 ค้นหา CBD (Network-Driven Detection)", expanded=True):
        ...
        w["accessibility"] = st.slider(
            "Accessibility (เข้าถึงง่าย)", 0.0, 1.0, float(w["accessibility"]), 0.05, disabled=locked
        )
        w["economic_activity"] = st.slider(
            "Economic Activity (กิจกรรมเศรษฐกิจ)", 0.0, 1.0, float(w["economic_activity"]), 0.05, disabled=locked
        )
        w["commercial_density"] = st.slider(
            "Commercial Density (ความหนาแน่นร้านค้า)", 0.0, 1.0, float(w["commercial_density"]), 0.05, disabled=locked
        )
        w["land_use"] = st.slider(
            "Land Use Diversity (ความหลากหลายการใช้ที่ดิน)", 0.0, 1.0, float(w["land_use"]), 0.05, disabled=locked
        )
        w["network_centrality"] = st.slider(
            "Network Centrality (โครงสร้าง Network)", 0.0, 1.0, float(w["network_centrality"]), 0.05, disabled=locked
        )
        StateManager.set_cbd_weights(w)
        if st.button("↺ รีเซ็ตน้ำหนักเป็นค่าเริ่มต้น", disabled=locked, key="_reset_cbd_weights_btn"):
            StateManager.set_cbd_weights(dict(CBD_DETECTION_CONFIG["cbd_score_weights"]))
            st.rerun()
