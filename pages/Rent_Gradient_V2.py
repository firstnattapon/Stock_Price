...
def safe_fetch_isochrone(api_key: str, travel_mode: str, ranges_str: str, 
    ...
    try:
        ...
        status_map = {
            401: "❌ Invalid API Key - Please check your Geoapify API key",
            403: "❌ API Key Forbidden - Check your account permissions",
            429: "⚠️ Rate Limit Exceeded - Please wait before retrying"
        }
        return None, status_map.get(response.status_code, f"API Error (Status {response.status_code})")
            
    except requests.Timeout: return None, "⏱️ Request Timeout"
    except requests.ConnectionError: return None, "🌐 Connection Error"
...
def render_sidebar():
    with st.sidebar:
        st.header("⚙️ GeoGIS Pro")
        
        # --- Config Management ---
        with st.expander("💾 Config Manager", expanded=False):
            ex_data = json.dumps({
        ...
        # --- Actions ---
        st.markdown("---")
        if st.button("🧩 Calculate Isochrones", type="primary", use_container_width=True):
        ...
        with st.expander("🕸️ Network Analysis", expanded=True):
            ...
            st.button("🚀 Analyze Roads", disabled=not has_iso, use_container_width=True, on_click=run_network_calc)
...
def run_isochrone_calc(active_markers):
    if not st.session_state.api_key: return st.toast("⚠️ Missing API Key")
    if not active_markers: return st.toast("⚠️ No active markers")
    ...
    with st.spinner("Calculating Coverage..."):
        ...
        for act_idx, (orig_idx, m) in enumerate(active_markers):
            ...
            if feats:
                ...
                st.toast(f"Point {orig_idx+1}: {err}", icon="⚠️")
        ...
        if features:
            ...
            st.toast("Isochrone Calculation Complete!", icon="✅")
        else:
...
def run_network_calc():
    ...
    status = st.status("🏗️ Analyzing Network Structure...", expanded=True)
    try:
