...
st.set_page_config(
    page_title="Geoapify Map (Chiang Khong)",
    page_icon="🌍",
    layout="wide"
)
...
if 'markers' not in st.session_state:
    # เริ่มต้น: ให้มีจุดเชียงของ 1 จุดเสมอ
...
st.title("🌍 Geoapify: เริ่มต้นที่เชียงของ")
    ...
    with col_btn1:
        if st.button("❌ ลบจุดล่าสุด", use_container_width=True):
            if st.session_state.markers:
                ...
                st.rerun()
    with col_btn2:
        # 🟢 แก้ไข: เปลี่ยนปุ่มล้าง เป็นปุ่มรีเซ็ตค่าเริ่มต้น
...
# --- 6. Display Map ---
def display_map():
    # ถ้ามีหมุด ให้โฟกัสที่หมุดสุดท้าย ถ้าไม่มี ให้โฟกัสที่เชียงของ
    if st.session_state.markers:
