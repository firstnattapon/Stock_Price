...
def initialize_session_state():
    """Initialize all session state variables, trying to load from URL first."""
    
    # กำหนดค่าเริ่มต้นแบบ Hardcoded (กรณีโหลด URL ไม่ได้)
    defaults = {
    ...
    # ตรวจสอบว่าเป็นการโหลดครั้งแรกหรือไม่ (เช็คจากตัวแปร markers)
    if 'markers' not in st.session_state:
        try:
            # พยายามโหลดไฟล์เริ่มต้นจาก URL
            response = requests.get(DEFAULT_JSON_URL, timeout=3)
            if response.status_code == 200:
                data = response.json()
                
                # อัปเดตค่า defaults ด้วยข้อมูลจาก URL
                defaults['markers'] = data.get("markers", defaults['markers'])
                defaults['isochrone_data'] = data.get("isochrone_data", defaults['isochrone_data'])
                defaults['intersection_data'] = data.get("intersection_data", defaults['intersection_data'])
                defaults['colors'] = data.get("colors", defaults['colors'])
                defaults['api_key'] = data.get("api_key", defaults['api_key'])
                defaults['map_style_name'] = data.get("map_style_name", defaults['map_style_name'])
                defaults['travel_mode'] = data.get("travel_mode", defaults['travel_mode'])
                defaults['time_intervals'] = data.get("time_intervals", defaults['time_intervals'])
                
        except Exception:
            # ถ้าโหลดไม่ได้ ให้ข้ามไปใช้ค่า Hardcoded defaults เงียบๆ
            pass

    # กำหนดค่าลงใน session_state
