...
# Keys to save in Export (Added 'show_traffic')
...
def initialize_session_state():
    ...
    defaults = {
        ...
        'show_traffic': False # [NEW] Traffic state
    }
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander("⚙️ ตั้งค่าแผนที่ & Layers", expanded=True):
            ...
            st.markdown("##### 🗺️ ข้อมูลพิเศษ")
            # [NEW] Traffic Checkbox
            st.checkbox("🚦 การจราจร (Google Traffic)", key="show_traffic")
            st.checkbox("👥 ความหนาแน่นประชากร (Longdo)", key="show_population")
            
            st.markdown("##### 🏙️ ผังเมือง & ที่ดิน")
...
def render_map():
    ...
    # 1. [NEW] Google Traffic Layer (Overlay)
    # Using "h" (hybrid) + "traffic" to ensure roads and traffic overlay correctly
