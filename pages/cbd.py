...
import matplotlib.pyplot as plt # [NEW] For plotting charts
import numpy as np # [NEW] For histogram calculation
...
def calculate_orientation_histogram(G, num_bins=36) -> Tuple[Any, Any]:
    """
    Calculate the bearing histogram for street orientation.
    Returns: (radii, theta) for plotting.
    """
    # Add bearing to edges
    ...
    n, _ = np.histogram(bearings, bins, weights=weights)
    ...
    # Align bars to North (rotate by 90 degrees/pi/2)
    # But usually Rose plots start East (0) counter-clockwise. 
    # Geographic bearings start North (0) clockwise.
    # We need to map compass bearings to polar coordinates carefully.
    # Simplification: Just plot raw bins mapping 0-360.
    
    width = (2*np.pi) / num_bins
    return n, theta, width

@st.cache_data(show_spinner=False, ttl=3600)
def process_network_analysis(polygon_wkt_str: str, network_type: str = 'drive') -> Dict[str, Any]:
    """
    Downloads OSM road network, calculates Centrality, Stats, and Orientation.
    """
    try:
        ...
        # --- [NEW FEATURE 1] Network Statistics ---
        stats = ox.basic_stats(G)
        enriched_stats = {
            "node_count": len(G.nodes),
            "edge_count": len(G.edges),
            "total_length_km": stats['edge_length_total'] / 1000,
            "intersection_count": stats['intersection_count'],
            "avg_street_length": stats['edge_length_avg']
        }
        
        # --- [NEW FEATURE 2] Street Orientation Data ---
        # We calculate histogram data here to keep the plotting in UI (or cache it)
        hist_data, theta, width = calculate_orientation_histogram(G)
        orientation_data = {
            "hist": hist_data,
            "theta": theta,
            "width": width
        } if hist_data is not None else None
        ...
        return {
            ...
            "stats": enriched_stats, # New
            "orientation": orientation_data # New
        }
...
def plot_orientation_chart(orientation_data):
    """Draw a Rose Plot using Matplotlib."""
    if not orientation_data:
        return
    
    hist = orientation_data['hist']
    theta = orientation_data['theta']
    width = orientation_data['width']
    ...
    bars = ax.bar(theta, hist, width=width, bottom=0.0, color='#FF8C00', alpha=0.8, edgecolor='black', linewidth=0.5)
    ...
    with st.sidebar:
        ...
        with st.expander("🕸️ วิเคราะห์โครงข่าย (Network Analysis)", expanded=True):
            ...
            if st.session_state.network_data:
                ...
                # --- [FEATURE 1: STATS] ---
                if 'stats' in net_data:
                    stats = net_data['stats']
                    st.markdown("##### 📊 สถิติโครงข่าย")
                    c1, c2 = st.columns(2)
                    c1.metric("ทางแยก (จุด)", f"{stats['intersection_count']:,}")
                    c2.metric("ถนนรวม (km)", f"{stats['total_length_km']:.1f}")
                    st.caption(f"ความยาวเฉลี่ย: {stats['avg_street_length']:.1f} ม./เส้น")
                
                # --- [FEATURE 2: ORIENTATION] ---
                if 'orientation' in net_data and net_data['orientation']:
                    st.markdown("##### 🧭 ทิศทางถนน (Orientation)")
                    # Layout: Text left, Chart right
                    oc1, oc2 = st.columns([0.4, 0.6])
                    with oc1:
                        st.caption("กราฟแสดงทิศทางการวางตัวของถนนในพื้นที่ (Rose Plot)")
                    with oc2:
                        plot_orientation_chart(net_data['orientation'])
                    
                st.markdown("---")

                if net_data.get('top_node'):
                    top = net_data['top_node']
                    st.markdown(f"**🏆 จุด Integration Center**")
